...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="olink[@role = 'crossref']" mode="class.value">
           <xsl:value-of select="'xref'"/>
    </xsl:template>

    <xsl:template match="filename[@class='devicefile']" mode="class.value">
    	<xsl:value-of select="'devicefile'"  />
    </xsl:template>

    <xsl:template match="filename[@class='directory']" mode="class.value">
